...
event_t *new_event(event_t *event)
{
    ...
    event_t *event_new = (event_t *)emalloc(sizeof(event_t));
    strcpy(event_new->description, event->description);
    strcpy(event_new->end_date, event->end_date);
    strcpy(event_new->location, event->location);
    event_new->repetition = event->repetition;
    strcpy(event_new->start_date, event->start_date);
    return event_new;
}
